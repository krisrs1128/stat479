...
subtitle: "Gaussian Processes for Regression"
format: revealjs
---
...
<img src="figures/stellar-variability_3_0.png"/>
...
- Amplitude related to spot coverage
...
We need a statistical model of functions that:
...
<img src="figures/gp_length_scale_interpretation.gif"/>
...
:::
::: {.column width="50%"}
<img src="figures/periodic_kernel_matrix.gif"/>
:::
::::
...
**Key difference from RBF:** Finite differentiability
...
We haven't yet explained how these samples are generated, but samples that use a Matérn kernel are often "rougher" are more realistic for real data.

:::: {.columns}
::: {.column width="50%"}
<img src="figures/gp-samples.gif"/>
:::
::: {.column width="50%"}
<img src="figures/matern_samples.gif"/>
:::
::::
...
GP: Estimate the entire function $f$.

Observation: This seems impossible, since functions are infinite-dimensional. But in any data, we only observe $f$ at finitely many points,
\begin{align*}
\mathbf{f}_{N} = \left(f\left(x_1\right), \dots, f(x_N))
\end{align*}
...
1. $\sigma_*^2$: epistemic (model uncertainty about $f$)
2. $\sigma_n^2$: aleatoric (irreducible measurement noise)

**Use case:** Forecast stellar flux at future times
...
1. [Optional]. Relate the previous parts to the expressions $p\left(y_{\ast} \vert x_{\ast}\right)$ and $p\left(y_{\ast} \vert x_{\ast}, \mathbf{y}, \mathbf{X}\right)$.
