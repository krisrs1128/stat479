...
![](figures/kernel_matrix_partition.png){fig-align="center"}
...
![](figures/conditioning_bivariate.png){fig-align="center"}
...
\begin{align*}
\text{Cov}(y, y') &= \mathbb{E}[\mathbf{x}^\top \beta \beta^\top \mathbf{x}'] \\
&= \mathbf{x}^\top \mathbb{E}[\beta \beta^\top] \mathbf{x}' \\
&= \mathbf{x}^\top (\sigma^2 \mathbf{I}) \mathbf{x}' \\
...
_Kernel answer_: Define closeness explicitly.
...
:::: {.columns}
::: {.column width="50%"}
![](figures/gp_samples.png)
:::
::: {.column width="50%"}
![](figures/matern_samples.png)
:::
::::
...
\begin{align*}
k_{\text{sum}}(\bs{x}, \bs{x}') = k_1(\bs{x}, \bs{x}') + k_2(\bs{x}, \bs{x}') (\text{   superposition})\\
k_{\text{prod}}(\bs{x}, \bs{x}') = k_1(\bs{x}, \bs{x}') \times k_2(\bs{x}, \bs{x}') (\text{   modulation})
\end{align*}
