...
degrades test set performance.
   ...
   ```{r}
   #| label: fit-replicates
   #| echo: false
   #| cache: true
   source("08-helpers.R")
   instability <- fit_replicates(B = 10)
   ```

   ```{r}
   #| label: instability-animation
   #| eval: false
   #| echo: false
   #| fig-width: 8
   #| fig-height: 4
   anim <- ggplot(instability, aes(x1, x2, fill = pred)) +
     geom_tile() +
     facet_wrap(~method) +
     ...
     transition_manual(replicate)
   p <- animate(anim, nframes = 20, fps = 10, width = 1000, height = 650, res = 100)
   anim_save(p, file = "figures/cart_instability.gif")
   ```
   ![](figures/cart_instability.gif){width=80%}
...
risks overfitting.
   ...
   #| fig.width: 9
   #| fig.height: 3.5
   diagonal_boundary_plot() + smooth_curve_plot()
   ```
...
1.  Draw $b = 1, \dots, B$ bootstrap samples from the training data,
   ...
   ![](figures/bagging_summary.png){width=80%}<br/>
   _Exercise: Consider an arbitrary sample $i$. In what fraction of bootstrap samples would you expect it to appear?_
...
cross-validation.  For sample $i$, set
  ...
  Predict using only,
  \begin{align*}
    \hat{f}_{\text{OOB}}^{(-i)}(x_i) &= \frac{1}{|B^{(-i)}|} \sum_{b \in B^{(-i)}} \hat{f}^b(x_i)
  \end{align*}
  ...
  \end{align*}
   ...
   ![](figures/oob_overview.png){width=90%}
...
of important variables:
   ...
   ![](figures/ensemble_vi.png){width=80%}

   _Exercise: TRUE FALSE The variable $j$ can have high importance $I_j$ even if it was never a root split in any tree._
...
depth-1 trees in the diagonal classification example shown at the top of this
note. With one split, the best we can hope for in any one tree is to split one
of the variables around 0.5. No amount of averaging will recover the diagonal.
...
1. **General algorithm**.
   ...
   _Exercise: TRUE FALSE In boosting, the trees are trained on bootstrap resampled versions of the original data._
...
variable, so the boosted model,
   ...
   ![](figures/ensembles_depth.png){width=90%}

   _Exercise: Imagine working at a company with both very large data and a strong parallel computing infrastructure. Can the trees $\hat{f}^{b}$ in boosting be trained in parallel? What about the $\hat{f}^{b}$ in bagging?_
...
$\mathbf{x}$ and feature $j$, the CP profile is:
   ...
   ![](figures/cp_profile_overview.png){width=70%}
