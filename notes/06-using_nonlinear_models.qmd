---
title: "Using Nonlinear Models"
subtitle: "Hyperparameter Tuning and Case Study"
format: revealjs
---

## Overview

### 6. Using Nonlinear Models
...
This has two competing terms -- data fit (first) vs. model complexity (second).
...
:::
::: {.column width="50%"}
<img src="figures/stellar_variability.png"/>
:::
::::
...
The original source `r Citep(bib, c("exoplanetGaussianProcess", "angus2017))` used a slightly more involved periodoc kernel.
It used a similar domain-informed kernel construction though.
...
**Input standardization:** $$\tilde{x} = \frac{x - \bar{x}}{\text{SD}[x]}$$
    ...
    - Must rescale $\ell \to \ell / \text{SD}[x]$

**Output centering:** $$\tilde{y} = y - \bar{y}$$

**Benefits:**
- Moves data away from "meaningful zeros"
- Makes default priors (e.g., $m = 0$) more sensible
...
- Interpretable: "\beta_{1}$ is the effect of $x_{1}$ all else held equazl"
...
- Interpretable Hyperparameters: $\ell$ (scale), $\sigma_f$ (amplitude), $p$ (period), ...
...
[GP Missing Data] Rerun the case study notebook with a window of observations missing. How does it
affect the posterior on the period? What do the posterior predictive samples
look like?
