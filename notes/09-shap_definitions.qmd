...
_Readings: [1](https://arxiv.org/abs/2207.07605), [2](https://proceedings.mlr.press/v108/janzing20a.html) (optional)_, _[Code](https://github.com/krisrs1128/stat479_notes/blob/master/notes/09-shap_definitions.qmd)_
...
subpopulation compared to another.
   ...
   _Exercise: What is one example (hypothetical, or from your own experience) where a local feature attribution method could be used? What would be the difference vs. obtaining a global variable importance?_
...
Equation @eq-shapley to arrive at a local feature attribution for sample $x$.
   ...
   _Exercise: Consider the four axioms for game theoretic SHAP. What do they imply about $\varphi_d(f, x)$?_
...
$x'$, and it's not obvious what a good choice of $D$ should be.
   ...
   ![](figures/shap_marginal_approach.png){width=90%}
...
1. Here, we're explaining the first 50 samples $x_1, \dots, x_50$ using the
...
sv = explainer.shap_values(X_explain)   # shape: (50, n_features)
   ```
...
1. If we stack the waterfall bars on top of one another and then compute the
same quantify for many samples, we obtain a beeswarm plot. The columns are
...
attributions, despite having similar predictions.
   ...
   ```{python}
#| label: shap-beeswarm
# Beeswarm plot: SHAP value distributions across all explained samples.
