...
1. $^\dagger$ It turns out that
  ...
  $$
   The right hand side is exactly the marginal expectation from equation
   @eq-marginal. So, the marginal approach is not just a computational
   convenience, it approximates a causally meaningful quantity that refelcts
   what it means to remove a feature's influence.
...
1. This is the analogous visualization if we stack the bars on top of one
