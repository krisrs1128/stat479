...
**Requirements.**
  ...
  - _Flexible inputs_. The features $x_{i}$ may be continuous or categorical
  types.
...
entire input space: $\mathcal{X} = R_{1} \cup R_{2} \cup \dots \cup R_{L}$.
   ...
   _Exercise: What tree does this partition correspond to? Make sure to annotate the regions $R_{l}$ and split rules._
...
training response in region $R_{l}$.
   ...
   _Exercise: The formula above has squared error terms like $\left(y_i - \bar{y}_{R^{\text{left}}}\right)^2$. How would you modify the figure to represent these quantities?_
...
\dots, L + 1\}$ and a fine grid of $s$ within each $R_{l}$. The best split is:
   $$
\left(l^*, s^*\right) := \arg \max_{l, s} \Delta\left(l, s\right)
$$
...
(l^*, j^*, s^*) := \arg\max_{l, j, s} \Delta(l, j, s)
   ...
   _Exercise: Compare and contrast the formulas in @eq-interval_delta and @eq-high_dim ._
...
of class $k$ samples in region $R_{l}$, these are defined as,
   ...
   $$
   \begin{align*}
\text{Gini}\left(R_{l}\right) &= \sum_{k= 1}^{K} \hat{p}_{lk}\left(1 - \hat{p}_{lk}\right)\\
...
(similarly to how a good regression tree learns regions $R_{l}$ with low RSS).
   ...
   _Exercise: What is the analogous figure when there are $K = 2$ classes?_
...
$Q - 1$ splits.
   ...
   candidate splits.
   ![](figures/sorted_1d_categories.png){width="450px"}
...
1. The tree growing algorithm may overfit, producing large trees with poor test
performance and interpretability. The diagram below illustrates this with a
one-dimensional dataset. Each step adds a split, shown as branching in the tree
diagram (top) and finer partitions in the function plot (bottom, pink line). The
last two splits add little value since the function is relatively flat in that
region.

   ![](figures/iterative_growing_tree.png){width="450px"}
...
conservative rule that rejects the first split would miss this signal entirely.
   ...
   _Exercise: Recall the definition of a feature interaction given at the start of these notes. How does this figure give an example of such an interaction?_
...
1. Let $T_0$ be the full tree. For penalty $\alpha \geq 0$, we find a subtree $T \subset T_0$ (formed by collapsing internal nodes) that minimizes:
   ...
   _Exercise: The figure above shows the tree $T_0$ from the original growing process and a version formed by collapsing at the internal node $t$._
    - _Which of the two trees has larger training error $\sum_{l=1}^{|T|} \sum_{i: x_i \in R_l} (y_i - \bar{y}_{R_l})^2$? If it cannot be determined, explain why._
    - _Which has larger $\left| T\right|$? If it cannot be determined, explain why._

1. $^{\dagger}$ For an internal node $t$ in the tree, define,
      - $T_t$: The subtree rooted at $t$ (node $t$ and all its
   descendants).
   ...
   \end{align*}
     $$
     where $\bar{y}_{T_{t}}$ is the average response among training samples in
     the subtree $T_{t}$. Below, this sums over all samples in the orange
     segment, measuring squared distances from each <span
     class="purple"/>$y_i$</span> to <span
     class="orange">$\bar{y}_{T_{t}}$</span>.
     - $\text{RSS}\left(T_t\right)$: The RSS when summing over leaves in the subtree $T_t$,
     $$
   \begin{align*}
   \text{RSS}\left(T_t\right) &= \sum_{l \in T_t}\sum_{i \in R_{l}} \left(y_i - \bar{y}_{R_{l}}\right)^{2}.
   \end{align*}
     $$
     Below, the outer sum loops over the three pink segments; the inner sum
     loops over points within each segment, measuring squared distances from
     each <span class="purple"/>$y_i$</span> to its<span
     class="pink"/>$\bar{y}_{R_{l}}$</span>.

   ![](figures/rss_tn_cart.png){width="450px"}

   _Exercise: For any given $t$, is $RSS\left(t\right) \geq RSS\left(T_t\right)$, $RSS\left(t\right) < RSS\left(T_t\right)$, or "it depends on $t$"?_
...
1. $^{\dagger}$ Keeping $T_t$ vs. collapsing it improves the RSS by $\text{RSS}(t) - \text{RSS}(T_t)$ at a cost of $|T_t| - 1$ extra leaves. The cost-benefit ratio is:
   ...
   ![](figures/alpha_defn_cart.png){width="500px"}
...
1. The link pruning algorithm applies to higher dimensions.  For example, the
figure below shows how collapsing a subtree simplifies a partition in
two-dimensions.
...
1. We fit a regression tree with default $\alpha$. Each leaf region $R_{l}$ predicts $\bar{y}_{R_{l}}$.
   ...
   This is the associated partition of the feature space $\mathcal{X} = \left[0, 1\right]^{2}$. Each leaf in the tree is one axis-aligned rectangle.
...
1. Let's study the influence of $\alpha$.  The `cp` hyperparameter in rpart is $\alpha$ normalized by the root node's RSS; the normalization makes `cp` comparable across datasets. We can see that when cp is small, we are left with a large tree (fine partition), while when cp is large, we have a simpler tree (coarse partition).
