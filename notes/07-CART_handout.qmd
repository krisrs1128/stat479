...
$R^{\text{left}}$ and $R^{\text{right}}$, divided at a threshold $s$. A natural
split point is the $s$ that minimizes:
...
1. So far we've focused on regression. For classification, we will still apply a
recursive splitting strategy, but we can't use $\Delta\left(l, j,
s\right)$ as our objective function.
...
1. So far we've only considered continuous features $x \in \reals^{J}$.  When
feature $j$ is categorical with $Q$ levels, say $x_{j} \in \{1, \dots, Q\}$, we
can't split at a threshold. Instead, we split levels into two groups.  Choose a
subset $A \subset \{1, \dots, Q\}$ and split depending on $x_{j} \in A$ or
$x_{j} \notin A$.
...
possible subsets of $A$ to consider.  For regression, we can reduce this to just
$Q - 1$ splits.
...
1. The tree growing algorithm we've defined might overfit to training data,
leading to poor test performance and reduced interpretability (the trees will be
quite large).
...
1. Let $T_0$ be the full tree from our greedy search. For a penalty $\alpha \geq
0$, we can look for a subtree $T \subset T_0$ (formed by collapsing internal
nodes) that minimizes:
...
1. $^{\dagger}$ The improvement when keeping $T_t$ vs.  collapsing it is
$\text{RSS}(t) - \text{RSS}(T_t)$. Since $T_t$ has $|T_t|$ leaves, the cost of
keeping the subtree is $|T_t| - 1$ extra leaves. The cost-benefit ratio is:
   $$
...
1. $^{\dagger}$ This suggests the final pruning algorithm.
...
1. Let's see how to use the `rpart` package to create regression trees. We'll
work with the 2D simulated dataset below. Notice that it isn't a simple
piecewise constant surface. There are also strong interactions between $x_1$ and
$x_2$.
...
1. Let's fit a regression tree with the default $\alpha$ penalty. Each leaf is a
region $R_{l}$ with constant prediction $\bar{y}_{R_{l}}$.
...
1. Sometimes we want to inspect individual nodes in our tree. Two helpful
objects are `fit$frame` and `fit$where`.  `fit$frame` gives summary statistics
about each node $R_{l}$, like the number of samples that descend from it, the
average value $\bar{y}_{R_{l}}$, and the $\alpha_{m}$ threshold that needs to be
reached before the node is pruned.
