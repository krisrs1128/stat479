...
$$
\sum_{l=1}^{\ell+1} \sum_{i: x_i \in R_l} (y_i - \bar{y}_{R_l})^2
$$
...
$$
\left(l^*, s^*\right) := \arg \max_{l = 1, \dots, \ell + 1} \Delta\left(l, s\right)
$$
...
rate. For example, the split below is still worth making, because the region
$R_{3}$ is likely to be generalize well.
...
1. So far we've only considered continuous features $x \in \reals^{D}$.  When
...
1. $^{\dagger}$ For an internal node $t$ in the tree, define,
   ...
   \end{align*}
     ...
     - $\text{RSS}\left(T_t\right)$: The RSS from keeping all leaves in $T_t$,
     $$
   \begin{align*}
...
1. $^{\dagger}$ The improvement from keeping $T_t$ vs.
collapsing it is $\text{RSS}(t) - \text{RSS}(T_t)$. Since $T_t$ has $|T_t|$
leaves, the cost of keeping the subtree is $|T_t| - 1$ extra leaves. The
cost-benefit ratio is:
   ...
   provide little benefit relative ot their complexity.
...
1. $^{\dagger}$ This suggests the final pruning algorithm.
   ...
   The result is a sequence of nested subtrees $T_0 \supset T_1 \supset \cdots
   \supset T_M$ with thresholds $0 = \alpha_0 < \alpha_1 < \cdots < \alpha_M$.
   For any penalty $\alpha \in [\alpha_m, \alpha_{m+1})$, the optimal subtree
   minimizing $\mathcal{L}_{\alpha}(T)$ is $T_m$.
