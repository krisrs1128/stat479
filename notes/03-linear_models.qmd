...
- Viability can be viewed as a **response variable** $y \in \mathbf{R}^{N}$, and the molecular variables can be treated as **features** $X \in \mathbf{R}^{N  \times J}$. Here
...
Imagine modeling viability $y_{i}$​ for sample $i$ as a linear function of a single gene's expression level $x_{i} \in \mathbf{R}$:
...
![](figures/Bias_and_variance_contributing_to_total_error.svg)
...
Respond to the following T/F questions from this week's reading. Justify your choices.
