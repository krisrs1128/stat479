...
_Reading [1](https://www.tidytextmining.com/topicmodeling.html) and [2](https://github.com/krisrs1128/stat479/blob/master/_posts/2021-03-30-week11-3/week11-3.Rmd), [Recording](), [Rmarkdown](https://github.com/krisrs1128/stat479/blob/master/_posts/2021-03-30-week11-3/week11-3.Rmd)_
...
mysterious formula, but it is just a function that is large when topic $l$ has
much larger probability than topic $k$ (see the figure).
...
```{r, fig.cap = "An illustration of the formula used for computing a word's discrimination between topics. The value of D is large when topic l has much larger probability than topic k."}
...
ggplot(df, aes(Var2, Var1)) +
  ...
  labs(
    x = expression(beta[kw]),
    y = expression(beta[lw])
  )
```
