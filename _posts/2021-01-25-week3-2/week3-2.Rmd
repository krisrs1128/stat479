...
2. Besides `menu` bindings, there are `checkbox`, `radio`, and `slider` inputs. If you revisit the gapminder visualization in [Lecture 1 - 3], you will see that the interaction was accomplished by binding a selection to a `slider` input.
3. A selection can combine multiple bindings. For example, we can filter by both MPAA rating and genre,
...
4. Interperetation 1: Dynamic queries create the visual analog of a database interaction. Rather than using a programming-based interface to `filter` elements or `select` attributes, we can design interactive visual equivalents.
5. Interpretation 2: Dynamic queries allow rapid evaluation fo conditional probabilities. The visualization above was designed to answer: What is the joint distribution of movie ratings, conditional on being a drama?
