...
simulated datasets are not even remotely plausible. It makes more sense to use a
subjective, informative prior in this case^[This situation is so extreme that
there is unlikely that even a frequentist would have an objection. However, in
less black-and-white situations, there might still be some debate about what
really to do.]
...
7. Note, the samples from the prior predictive do not need to look exactly like
the observed data — if they did, there would be no need to fit model parameters!
...
datasets <- list(
  ...
  "clusters" = with(
    GM@data, 
    list(N = length(log_pm25), y = log_pm25, x = log_sat, group = cluster_region, R = n_distinct(cluster_region))
  )
)

model_def <- list(
  "lm" = stan_model("~/Desktop/lm.stan"),
  "hier" = stan_model("~/Desktop/hierarchical.stan")
)
```
...
```{r}
save(models, file = "~/Desktop/models.rda")
#models <- get(load("~/Desktop/models.rda"))
```

12. We can verify that features of the real dataset are accurately captured by
features of the posterior predictive.

```{r, fig.width = 10, fig.height = 6}
posterior_samples <- as.matrix(models$clusters, pars = "log_pm_rep")[100:111, ] %>%
  t() %>%
  as_tibble() %>%
  bind_cols(GM@data) %>%
...
13. One subtlety is that there is a danger of overfitting features in the
...
evaluate the posterior predictive using the slope, since by definition this will
be well-captured).
