...
1. In reality, data tend not to be as clearly separated into a fixed number of
distinct clusters. More often, even once a cluster formed, it's possible to
identify a few subclusters. For example, if you initially clustered movies into
"drama" and "scifi", you might be able to further refine the scifi cluster into
"time travel" and "dystopian future."

2.$K$-means only allows clustering at a single level of ``magnification.'' To
simultaneously cluster across scales, you can use a hierarchical clustering. As
a first observation, note that a tree can be used to implicitly store many
clusterings at once. You can get a standard clustering by ``cutting'' the tree
at some level.
...
```{r, fig.cap = "At initialization, the hierarchical clustering routine has a cluster for each observation."}
include_graphics("https://drive.google.com/uc?id=1-VCD5H7YZMLlgCsm02qHCqZO-l0xadRo")
```
	
```{r, fig.cap = "Next, the two closest observations are merged into one cluster. This is the first merge point on the tree." }
include_graphics("https://drive.google.com/uc?id=1bxFQlxfq2BZ6owPw3K2Ft5e7jE6eB6HE")
```
	
```{r, fig.cap = "We continue this at the next iteration, though this time we have compute the pairwise distance between all clusters, not observations (technically, all the observations were their own cluster at the first step, and in both cases, we compare the pairwise distances between clusters)."}
include_graphics("https://drive.google.com/uc?id=1Jcmlq5qFuNA42IRruIKqlYAc8JM5wys8")
```

```{r, fig.cap = "We can continue this process..."}
include_graphics("https://drive.google.com/uc?id=1kYqnHe_ARcpYmaMGlwQq9uAM7urGdzjn")
```

```{r, fig.cap = "... and eventually we will construct the entire tree."}
...
8. We can customize our tree visualization using ggraph. First, we need to
convert the hclust object into a ggraph, using the same `as_tbl_graph` function
we used in the network and trees lectures.
