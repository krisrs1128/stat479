...
description: |
  A discussion of the key terminology in ggplot2, and an example of iteratively
  refining a simple scatterplot.
author:
  - name: Kris Sankaran
...
Reading: Introduction to Data Science, [Chapter 7](https://rafalab.github.io/dsbook/ggplot2.html)

* [Recording]
* [Rmarkdown](https://raw.githubusercontent.com/krisrs1128/stat479/master/_posts/2021-01-12-week1-2/week1-2.Rmd)

[ggplot2](https://ggplot2.tidyverse.org/) is an R implementation of the _Grammar
of Graphics_. The idea of the Grammar is to define the basic "words" from which
visualizations are built, and then let users compose them in original ways. This
is in contrast to systems with prespecified chart types, where the user is
forced to pick from a limited dropdown menu of plots. Just like ordinary
language, once you know how to speak the grammar of graphics, you will be able
to express ideas that no one else has had before.
...
Your reading for this lecture constructs this plot, and we're going to recreate
it in these notes.
...
We'll be experimenting with a few of these in the third lecture this week.

### Aesthetic mappings

Aesthetic mappings make the connection between the data and the geometry. The
physical properties of a geom’s mark is determined by values in the
corresponding observation's data field, as specified by the aesthetic mapping.

Deconstructing the original graph further, we recognize these links between the
abstract data fields for each state and the circles' visual properties,

* State Population <--> $x$-axis coordinate
* Number of murders <--> $y$-axis coordinate
* Geographical region <--> color

We can establish this mapping in the plot using the `aes` function. Notice that
we don't need to quote the column names within this command -- it knows to refer
back to the `murders` data frame in `ggplot(murders)`.
...
Once nuance is that scales aren't limited to these sort of $x$ and $y$
transformations. They can be applied to modify any relationship between a data
field and its appearance on the page. For example, this changes the mapping
between the region field and circle color.
...
The problem with this graph is that it doesn't allow us to compare individual
states, only regions. For that visual task, we'll need text labels. We can
encode the coordinates for these marks again using `aes`, but this time within
a `geom_text` layer.
...
-- we could pass in `label` to a `geom_text` label, but that wouldn't make sense
for a `geom_point` layer.

It's usually a good thing to make your code as concise as possible. For ggplot2,
one way you can accomplish this is by sharing elements from the `aes` mapping
across layers (e.g., not having to type `population` and `total` twice). This
can be done by defining a "global" aesthetic, putting it inside the top `ggplot`
call.
...
How can we improve the readability of this plot? From the intro figure, you
might already have ideas,
...
For 1., the `ggrepel` package can handle the computation needed to find better
...
For 2., let's first compute the national murder rate (per million),
...
within a `guide`.
* Re-order the regions in the legend. Hint: Reset the factor levels in the
`region` field of the `murders` data.frame.
* Only show labels for a subset of states that are far from the national rate.
Hint: Filter the `murders` data.frame, and use the `data` field specific to the
