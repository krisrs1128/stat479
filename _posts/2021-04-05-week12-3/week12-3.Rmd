...
4. The simplest model simply fits $\~station = a + b \* satellite$ at locations
where they are both available. This model was used in practice by the Global
Burden of Disease project until 2016.

```{r}
ggplot(GM@data, aes(log_pm25, log_sat)) +
...
improve the model fit?
  	* Remember, the error $\epsilon_{i}$ in a model $y_i = f\left(x_{i}\right) +
  	\epsilon\_{i}$ is just a stand in for our ignorance^[As a thought experiment,
  	if we knew the position and velocity of every atom in the universe, we would
  	not need $\epsilon\_{i}$.] of the myriad of unmeasured factors that determine
