...
_[Reading](https://search.library.wisc.edu/catalog/9911196629502121) (Chapter 9), [Recording](), [Rmarkdown](https://raw.githubusercontent.com/krisrs1128/stat479/master/_posts/2021-03-06-week8-2/week8-2.Rmd)_
...
them. The dataset below is from a friendship network from a survey of high
schoolers in 1957 and 1958, from the book "Introduction to Mathematical
Sociology."
...
3. In either trees or networks, attributes of the nodes and edges can be encoded
...
6. One common layout algorithm uses « force-directed placement. » The edges here are interpreted as physical springs, and the node positions are iteratively updated according to the forces induced by the springs.
...
Bundled connections can be visualized using the `geom_conn_bundle` geometry.
Before using this layout, it is necessary to have a hierarchy over all the
nodes, since shared ancestry among connected nodes is how the proximity of paths
is determined.
	
```{r, layout='l-page', fig.width = 12, fig.height=12, fig.cap = "An example of hierarchical edge bundling in R."}
