...
cell viability prediction problem studied in these papers: [1, 2]. You can
download the primary source data from [this
...
rho <- cor(X)
diag(rho) <- 0
corr_ix <- which(rho > 0.98 * max(rho), arr.ind = TRUE)
v1 <- colnames(X)[c(4772, 4723)]
ggplot(X) +
    geom_point(aes(.data[[v1[1]]], .data[[v1[2]]])) +
    labs(title = "Example Correlated Genes")
```
