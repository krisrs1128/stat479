...
input space. This is especially easy to visualize in two dimensions.
    ...
    a. For the partition in @fig-partition, sketch the corresponding decision tree.

    b. For the tree in @fig-tree, sketch the corresponding partition.

   ::: {#fig-partition-tree layout-ncol=2}
   ![Partition example](figures/partition_example-pair.svg){#fig-partition}
   ![Tree example](figures/tree_example-pair.svg){#fig-tree}
   :::
...
compositionally.
   ...
   each kernel $k(x, x')$ and interpret each hyperparameter geometrically or
   analytically.  Fix hyperparameters and plot the function $x \mapsto k(x, 1)$
   for each kernel.  Explain: what does this cross-section $k(\cdot, x_0)$
   represent?
