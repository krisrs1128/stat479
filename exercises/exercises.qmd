...
features using the lasso objective:
   ...
   where $N = 330$ samples, $y_n = \log(\text{ozone})_n$, and $x_n \in
   \mathbb{R}^{45}$ contains weather features and all pairwise interactions.
   @fig-lasso-plots displays cross-validation error (left) and coefficient paths
   (right) as functions of $\lambda$.

   ```{r}
   #| label: fig-lasso-plots
   #| fig-cap: Lasso cross validation error and coefficient paths, for [Interpreting Lasso Regularization Paths].
   #| echo: true
   library(faraway)
   library(glmnet)
   data(ozone)
   ...
   b. Explain the performance and sparsity tradeoffs visible in these plots. As $-\log \lambda$ increases (i.e., $\lambda$ decreases), how do test error and
   sparsity change? Point to specific features in both panels that support your
   answer.
...
choices.
   ...
   ```{r, eval=FALSE}
   library(glmnet)
   prostate <- read.csv("https://hastie.su.domains/ElemStatLearn/datasets/prostate.data", sep = "\t")
   ...
   * Refit the lasso to $\left(\mathbf{y}^{b}, \mathbf{X}^{b}\right)$, using CV to tune $\lambda$ each time
...
1. [GP Conditioning] Consider a GP with zero mean function and RBF kernel $k(x, x′)
= \exp(−\|x − x′\|^2)$. You have two test points $x^* = [0, 2]^\top$ at which
you want predictions, but no training data.

    a. What is the mean vector and covariance matrix of the prior distribution $p(\mathbf{f}_{*})$ at the test points.

    b. Now suppose you observe one training point $(x_1, y_1) = (1, 0.5)$ with noise variance $\sigma^2_n = 0.01$. Describe qualitatively (without full calculation) how the posterior mean and variance at each test point will change compared to the prior.

    c. If you instead had $\sigma^2_n$ = 100, how would your answer to part (b)
    change?

1. [GP True/False] Select all the TRUE statements about GPs below.

    a. TRUE FALSE The posterior variance at a test point x* is always smaller than
    the prior variance at that point, regardless of where the training data are
    located.

    b. TRUE  FALSE Multiplying two kernels $k^{\text{new}}(x, x') = k_{1}(x, x')k_{2}(x, x')$ always
    results in a valid positive semi-definite kernel.

    c. TRUE FALSE The posterior variance at a test point $x^*$ is always less than
    or equal to the prior variance at that point, regardless of the distances to the
    training data.

    d. If $x$ and $x'$ are far apart relative to the lengthscale $\ell$, then
    the corresponding function values $f(x)$ and $f(x')$ are necessarily
    independent.

    e. For a stationary kernel, the correlation between function values depends
    only on $\|x − x'\|$, not on the absolute locations $x$ and $x'$.
...
1. [Intrinsic Interpretability Portfolio] Apply two intrinsic interpretability
methods to a problem of your choice. The essential task: develop critical
judgment about what works through direct experience.
   ...
   d. **Comparison.** Which conclusions persist across methods? Which differ,
   and why might this occur? Include 1 - 2 figures to justify your conclusions.
   ...
   e. **Sanity Check.** Test the reliability of one model. Acceptable checks:
        ...
        * Bootstrap data; check stability
        ...
        If you want to try an alternative check, you may contact the
        instructors.
...
1. [Boosting Iterations] Consider boosting on data from $y = 3\sin(x) + \epsilon$, $\epsilon \sim \mathcal{N}(0, 0.25)$.
   ...
   ::: {#fig-boosting-steps fig-cap="Example result for [Boosting Initial Steps]." width="40%"}
