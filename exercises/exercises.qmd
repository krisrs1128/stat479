...
1. [Interpreting Lasso Plots] Figure \ref{fig:} shows the results of a lasso
model fit to predict ozone concentration based on weather features, like
temperature and humidity.  Specifically, we solve the optimization

    $$\min_{\beta_0, \beta} \left( \frac{1}{2N} \sum_{n=1}^{N} (y_n - \beta_0 - x_n^T \beta)^2 + \lambda \|\beta\|_1 \right)$$

    where $y$ is the log ozone concentration and $x_{i}\in \mathbf{R}^{45}$ contains
    all weather features and their first-order interactions. There are $N = 330$
   samples total. This problem asks you to explain the output in precise mathematical terms.
   ...
   a. The left-hand panel shows the cross validation error as a function of
   $\lambda$. The two dashed lines correspond to $\lambda_{\text{min}}$ and
   $\lambda_{1\text{se}}$. Which is which? Explain when to prefer one choice
   over the other.

   b. As we increase $-\log\lambda$, how does the selected model's bias and
   variance change? Refer to the figure in your explanation.

   c. Consider the blue path in the right-hand panel. Notice that its magnitude
   initially increases, but around $−\log\lambda\approx 5$, it decreases, even
   though the penalty $\lambda$ is getting weaker. Why might this occur, and
   what does it suggest about the relationship across predictors?

   d. As $-\log\lambda$ increases, the paths in the right-hand panel converge to
   a specific set of values. What are these values?

1. [Cross-Validation for Lasso] Consider the lasso estimator
...
choices.
   ...
   with low variance but high bias in its predcitions. Repeat the exercise for
...
1. [Prior and Posterior Predictive Distributions] The prior and posterior
predictive distributions describe plausible functions before and after seeing
the data, respectively. This problem asks you to compare these distributions on
a simulated example.

   a. Before seeing any data, we encode our prior beliefs using the generative
   mechanism below. This is a GP with an RBF kernel whose hyperparameters are
   drawn from log-normal priors.

   \begin{align*}
    A &\sim \mathrm{LogNormal}(-1, 1) \\
    \ell &\sim \mathrm{LogNormal}(0, 1) \\
    f(\cdot) \mid A, \ell &\sim \mathrm{GP}\!\left(
    0,\;
    A^2 \exp\!\left(-\frac{(x-x^{\prime})^2}{2\ell^2}\right)
    \right). \\
   \end{align*}
   When using `greta.gp`, this can be encoded using the block below. Explain
   what would be the effect of increasing the first argument to `lognormal` call
   used to define `amplitude`. Repeat this for `lengthscale` and `sigma`.
   ...
   b. Draw 50 samples from the prior predictive distribution $p\left(f\right)$.

   c. Imagine we have now sampled the data $(x_{n}, y_{n})$ visible in Figure
   \ref{fig:predictive_data}, which can be downloaded at [this
   link](https://github.com/krisrs1128/stat479/blob/master/data/prior_posterior_example_data.csv).
   Use these data to draw 50 samples from the posterior predictive $p\left(f
   \mid \left(x_{n}, y_{n}\right)\right)$.
   ...
   d. Visualize the results from parts (b) and (c). Your results should look
   similar to Figure \ref{fig:predictive_example}. Comment on the differences
   between the prior and posterior predictive samples.
